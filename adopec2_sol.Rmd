...
sum(is.na(counts))
```
```{r iniplot, echo=FALSE, fig.width=7,fig.height=6,fig.cap="\\label{fig:iniplot}Análisis exploratorio de las muestras."}
...
El siguiente paso es transformar los datos para estabilizar la varianza y poder así estudiarlos visualmente más fácilmente.
```{r include = FALSE}
...
Enriquecemos los datos con la información sobre el gen al que pertenece cada `ensembl`. Para ello utilizamos la base de datos `org.Hs.eg.db`. Eso nos permite ver cuales son los genes más diferencialmente expresados en cada caso.  

Después guardamos los archivos con los resultados enriquecidos y ordenados por p-adjust.
...
```
Por último utilizamos la función `enrichKEGG` del paquete *clusterProfiler* para obtener pathways representativos de los genes más sobre expresados. En los apéndices se puede ver los dos pathways más representativos para el contraste ELIvNIT.  
...
\begin{center}Fig.1: Boxplot de los datos normalizados\end{center}
 
![Boxplot Norm](figures/boxplotnorm.png)
\begin{center}Fig.2: Gráficos de dispersión de transformaciones de la varianza\end{center}
 
![Boxplot Norm](figures/vst.png)

\begin{center}Fig.2: Principal Component Analysis de los datos normalizados\end{center}
 
![Boxplot Norm](figures/pca.png)



\begin{center}Fig.2: Pathway Primary Immunodeficiency \end{center}
 
![Boxplot Norm](figures/primary_im.png)
\begin{center}Fig.3: Pathway Hematopoietic cell lineage \end{center}
 
![Boxplot Norm](figures/Hematopoietic.png)
